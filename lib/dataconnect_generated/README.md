...
```dart
// No required arguments
IkPharmaConnector.instance.getParacetamolProducts().execute();
```



#### Return Type
`execute()` returns a `QueryResult<GetParacetamolProductsData, void>`
```dart
/// Result of an Operation Request (query/mutation).
class OperationResult<Data, Variables> {
  OperationResult(this.dataConnect, this.data, this.ref);
...
final result = await IkPharmaConnector.instance.getParacetamolProducts();
GetParacetamolProductsData data = result.data;
final ref = result.ref;
```
...
An example of how to use the `Ref` object is shown below:
```dart
final ref = IkPharmaConnector.instance.getParacetamolProducts().ref();
ref.execute();
